...
if [[ `grep $site ~ebaluyut/gmtsar-aux/txt_files/site_sats.txt | grep $sat | grep $trk | grep $frame | wc -l` -gt 0 ]]
then
...
echo "$site $sat $trk $frame" >> ~ebaluyut/gmtsar-aux/txt_files/site_sats.txt
if [[ "$sat" == "TSX" ]]
then
  echo "$site TDX $trk $frame" >> ~ebaluyut/gmtsar-aux/txt_files/site_sats.txt
fi

scp ~ebaluyut/gmtsar-aux/txt_files/site_sats.txt $t31/ebaluyut/gmtsar-aux/txt_files/
