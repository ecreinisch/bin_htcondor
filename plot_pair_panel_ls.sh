#!/bin/bash
# script for plotting 1 pair with footer information for left side plots in a panel
# Elena C Reinisch 20170520
# update ECR 20180319 update for new bin_htcondor repo

if [[ $# -eq 0 ]]
then
echo "script for plotting 1 pair with footer information on left side of a panel plot"
...
then
  ...
  then
    grdimage $pha1 -Y7 -C${cdir}/cpt.cpt -JX7/12 $region -P -K -Bx${dlon} -By${dlat} -BWSne > ${outfile}
...
then
  ...
  if [[ $isutm == 0 ]] # if not UTM, plot with file's region, dlat, and dlon
  then
    grdimage r2mm.grd -Y7 -C${cdir}/cpt.cpt -JX7/10 $region -P -Bx${dlon} -By${dlat} -BWSne -O -K > ${outfile}
  else
...
then
  ...
  if [[ $isutm == 0 ]] # if not UTM, plot with file's region, dlat, and dlon
  then
    grdimage r2mm.grd -X7.5 -C./cpt.cpt -JX7/12 $region -P -Bx${dlon} -By${dlat} -BWSne -K > ${outfile}
  else
  ...
  if [[ $isutm == 0 ]] # if not UTM, plot with file's region, dlat, and dlon
  then
    grdimage $pha1 -Y7 -C${cdir}/${cptname} -JX10/10 $region -P -K -Bx${dlon} -By${dlat} -BWSne > ${outfile}
  else
...
if [[ $isutm == 1 ]]
then
  psbasemap -JX7/12 $region_km -Bx${dlon_km}+u"km" -By${dlat_km}+u"km" -BWSne -P -K -O >> ${outfile} 
fi
