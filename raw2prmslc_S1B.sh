...
if [[ $# -eq 0 ]]
then
  echo "script for pre-processing S1B data; to be used on porotomo for getting pair list metadata (not for forming actual pairs)"
  echo "run in raw directory"
  echo "raw2prmslc_S1B.sh [trk] [site] [list of directories]"
  echo "e.g."
  echo "raw2prmslc_S1B.sh T144 brady preproc_porotomo.lst"
  exit 1
fi
...
while read -r a b; do
  if [[ ! -d $a ]]
  then
     scp -r $maule:/s21/insar/S1B/${trk}/raw/$a .
     cp $a/$b .
  fi
done < $3
...
echo msafe_dir = $c
echo mEOFfile = $d
mbase_name=`find ../${msafe_dir}/annotation -name "s1b*00${subswath}.xml" | awk -F/ '{print $(NF)}' | awk -F. '{print $1}'`
#awk 'NR>1 {print $0}' < ../${msafe_dir}/manifest.safe > tmp_file
#cat ../${msafe_dir}/annotation/${mbase_name}.xml tmp_file /t31/ebaluyut/GMTSAR_ShortCourse/Example_S1B_Stack_CPGF_T173/raw_orig/s1b-aux-cal.xml > ./${mbase_name}.xml
cat ../${msafe_dir}/annotation/${mbase_name}.xml > ./${mbase_name}.xml

# set up symbolic links
ln -s ../${msafe_dir}/measurement/s1b*00${subswath}.tiff .
ln -s ../${msafe_dir}/${mEOFfile} .
while read -r a b; do
safe_dir=$a
EOFfile=$b
echo EOFfile = $b
base_name=`find ../$a/annotation -name "s1b*00${subswath}.xml" | awk -F/ '{print $(NF)}' | awk -F. '{print $1}'`
#awk 'NR>1 {print $0}' < ../$a/manifest.safe > tmp_file
#cat ../$a/annotation/${base_name}.xml tmp_file /t31/ebaluyut/GMTSAR_ShortCourse/Example_S1B_Stack_CPGF_T173/raw_orig/s1b-aux-cal.xml > ./${base_name}.xml
cat ../${safe_dir}/annotation/${base_name}.xml > ./${base_name}.xml

# set up symbolic links
ln -s ../$a/measurement/s1b*00${subswath}.tiff .
