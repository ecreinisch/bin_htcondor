...
if [[ `grdinfo $grdfile | grep UTM | wc -l` -gt 0 ]] || [[ $grdfile == *"utm"* ]]
then
...
if [[ "$grdfile" == *"phase"* ]]
then
  if [[ `grdinfo $grdfile | grep radian | wc -l` -gt 0 ]]
  then
  ...
  then
    ...
    grdimage $grdfile -Y7 -C${cdir}/cpt.cpt $jflag $region -P -K -Bx${dlon} -By${dlat} -BWSne+t"${sat1} ${trk1} ${pair1}" > ${outfile}
  else # if UTM plot file without labeling anything
   grdimage $grdfile -Y7 -C${cdir}/cpt.cpt $jflag $region -P -K > ${outfile}
  fi
...
elif [[ "$grdfile" == *"avg_range"* ]] || [[ "$grdfile" == *"stack"* ]] || [[ "$grdfile"* == *"summed"* ]]
then
  #grdmath $grdfile ISFINITE $grdfile MUL PI DIV 2.0 DIV $mmperfringe  MUL = r2mm.grd
  grdmath $grdfile 1000  MUL = r2mm.grd
  zmin=`grdinfo -C -L2 r2mm.grd | awk '{print $6}'`
  zmax=`grdinfo -C -L2 r2mm.grd | awk '{print $7}'`
  dz=`echo $zmax $zmin | awk '{print ($1-$2)/50}'`
  # 20180510 SAB changed makecpt to grd2cpt for unwrapped phase plot 
...
# file is unwrapped m 
elif [[ "$grdfile" == *"drho"* ]] || [[ "$grdfile" == *"range"* ]]
then
  grdmath $grdfile 1000 MUL = r2mm.grd # convert to mm
  zmin=`grdinfo -C -L2 r2mm.grd | awk '{print $6}'`
  zmax=`grdinfo -C -L2 r2mm.grd | awk '{print $7}'`
  dz=`echo $zmax $zmin | awk '{print ($1-$2)/50}'`
...
then
  zmin=`grdinfo -C -L2 $grdfile | awk '{print $6}'`
  zmax=`grdinfo -C -L2 $grdfile | awk '{print $7}'`
  dz=`echo $zmax $zmin | awk '{print ($1-$2)/50}'`
  #20180510 SAB changed makecpt to grd2cpt for unwrapped phase plot 
  ...
  if [[ $isutm == 0 ]] # if not UTM, plot with file's region, dlat, and dlon
  then
    grdimage $grdfile -X7.5 -C./cpt.cpt -JX7/12 $region -P -Bwsne -K > ${outfile}
  else
    grdimage $grdfile -X7.5 -C./cpt.cpt -JX7/12 $region -P -K > ${outfile}
  fi
...
then
  ...
  if [[ $isutm == 0 ]] # if not UTM, plot with file's region, dlat, and dlon
  then
    grdimage $grdfile -X7.5 -Y5 -C./cpt.cpt $jflag $region -P -Bwsne -K > ${outfile}
  else
    grdimage $grdfile -X7.5 -Y5 -C./cpt.cpt $jflag $region -P -K > ${outfile}
  fi
 ...
 # rm cpt.cpt
  ...
  if [[ $isutm == 0 ]] # if not UTM, plot with file's region, dlat, and dlon
  then
    grdimage $grdfile -Y7 -C${cdir}/${cptname} ${jflag} $region -P -K -Bx${dlon} -By${dlat} -BWSne+t"${sat1} ${trk1} ${pair1}" > ${outfile}
  else
    grdimage $grdfile -Y7 -C${cdir}/${cptname} ${jflag} $region -P -K > ${outfile}
  fi 
...
then
  ...
  # cat ${site}_faults_utm.txt | awk '{print $1,$2}' | psxy ${region} -J -W1 -O -K -V -P >> ${outfile}
  # cat ${site}_fumaroles_utm.txt | awk '{print $1,$2}' | psxy ${region}  -J -Sd0.25 -W.75  -O -K -V -P >> ${outfile}
    cat ${site}_gps_utm.txt | awk '{print $1,$2}' | psxy ${region} -J -Sa0.1 -Gblack -O -K -V -P >> ${outfile}
    cat ${site}_gps_utm.txt | awk '{print $1,$2, substr($5,5,2)}' > text.tmp
    pstext text.tmp -R -F+jMC+f3p -J -Gwhite -N -O -K  >> ${outfile}
    rm text.tmp
...
# add footer info
if [[ "$grdfile" != *"stack"* && "$grdfile" != *"avg_range"* && "$grdfile" != *"summed"* ]]
then
