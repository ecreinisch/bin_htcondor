#!/bin/bash
# when run in S1B downloads directory, generates list of orders to date and outputs text file S1B_OrderList.txt
...
then
    echo "gen_submitted_order_list_S1B.sh"
    echo "when run in a directory containing order receipts, generates list of orders to date"
    echo "outputs S1B_Orders file with columns of #date site sat track swath frame orbit ascdes status source filename path url"
...
then
  ...
  cp `ls -d ../S1B_OrderList.txt | tail -1` Submitted_Orders.txt
  cp ../S1B_OrderList.txt ../Archived_OrderLists/S1B_Orders-`date +%Y%m%d_%H%M`.txt
...
while read -r a; do
   ...
   then
     ...
     then
       ...
       elif [[ ! -z `find ../${trk}/raw -name "S1B*${scene_date}*${orbit}*" ` ]]
       then
         estatus=D
         dirname=`find ../${trk}/raw -maxdepth 1 -name "S1B*${scene_date}*${orbit}*" -type d | awk -Fraw/ '{print $2}' | awk -F/ '{print $1}'`
         data_loc2=/s21/insar/S1B/${trk}/raw/${dirname}
         data_loc="\/s21\/insar\/S1B\/${trk}\/raw\/${dirname}"
       ...
       then
         ...
         mv $dirname /s21/insar/S1B/${trk}/raw/
         data_loc="\/s21\/insar\/S1B\/${trk}\/raw\/${dirname}"
         mkdir -p untarred
    ...
    # check to see if downloaded but metadata missing
     ...
     if [[ ! -z `find ../${trk}/raw -name "*S1B*${scene_date}*${orbit}*" ` ]]
       then
         estatus=D
         dirname=`find ../${trk}/raw -maxdepth 1 -name "*S1B*${scene_date}*${orbit}*" -type d | awk -Fraw/ '{print $2}' | awk -F/ '{print $1}'`
         data_loc="\/s21\/insar\/S1B\/${trk}\/raw\/${dirname}"
   ...
   then
     if [[ ! -z `grep ${scene_date} Cancelled_Orders.txt` ]]
       ...
       then
         ...
         estatus=D
         dirname=`find ../${trk}/raw -maxdepth 1 -name "*S1B*${scene_date}*${orbit}*" | awk -Fraw/ '{print $2}' | awk -F/ '{print $1}'`
         filename=${dirname}.zip
         data_loc="\/s21\/insar\/S1B\/${trk}\/raw\/${dirname}"
         data_loc2=/s21/insar/S1B/${trk}/raw/${dirname}
         #frame=`echo $dirname | awk -F- '{print substr($1, length($1)-3, 4)}'`
        # orbit=`grep absOrbit ${data_loc2}/T*B/T*${epoch_date}*/T*${epoch_date}*.xml | awk -F\> '{print $2}' | awk -F\< '{print $1}'` # consider adding lines to untar filename and move directory to specified location, so we can update url and data_loc, and then we can grep for orbit
     elif [[ ! -z `find . -maxdepth 1 -name "S1B*${scene_date}*${orbit}*.zip"` ]]
       then
         echo "data in zip file"
         filename=`find . -maxdepth 1 -name "S1B*${scene_date}*${orbit}*.zip" | awk -F/ '{print $2}'`
...
if [[ "$arch" == 1 ]]
then
  find /s21/insar/S1B/*/raw -name "S1B*" -type d > s1a.tmp
  while read -r a; do
  data_loc2=$a
  scene_date=`echo $data_loc2 | awk -F/ '{print $(NF)}' | awk -F_  '{print $6}' | awk -FT '{print $1}'`
  path=`echo $data_loc2 | sed 's/../\/s21\/insar\/S1B/'`
...
# clean up output 
#sort Submitted_Orders.txt | sed '1s/^/#date site sat track swath frame orbit ascdes status source filename path url/' | column -t  > ../S1B_Orders-`date +%Y%m%d_%H%M`.txt
#head -1 Submitted_Orders.txt ; tail -n +2 Submitted_Orders.txt | sort -t : -k1,1 -k2,2n | column -t  > ../S1B_Orders-`date +%Y%m%d_%H%M`.txt
...
#column -t Submitted_Orders.txt > ../S1B_Orders-`date +%Y%m%d_%H%M`.txt
column -t Submitted_Orders.txt > ../S1B_OrderList.txt


# change permissions of files for future users
chmod a+r+w+x Submitted_Orders.txt
chmod a+r+w+x OrderList
#chmod a+r+w+x ../S1B_Orders-`date +%Y%m%d_%H%M`.txt
chmod a+r+w+x ../S1B_OrderList.txt
