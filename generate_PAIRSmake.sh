...
# edit Kurt and Sam 20211025 replaced with appropriate language and logic of -r and -s
# edit Kurt and Sam 20211108 add new search options
# edit Kurt 20230624 options -1 and -2 do not work
...
#  echo "-1[val] : print pairs with ref equal to [val] in yyyymmdd format"
#  echo "-2[val] : print pairs with sec equal to [val]  in yyyymmdd format"
#  echo "-b[val] : print pairs with abs. perpendicular baseline <= [val]"
 echo "-d[val] : print pairs with time interval equal to [val] in days"
 echo "-e[val] : print pairs designated in file [val]; format of [val] should have a column of ref epochs in yyyymmdd format and a column of sec epochs in yyymmdd format"
 ...
 echo "-n[val] : print pairs that are NOT designated in file [val]; format of [val] should have a column of ref epochs in yyyymmdd format and a column of sec epochs in yyymmdd format.  This is most likely to be used on submit-2 to re-run some pairs of the dataset."
 echo "-o[val] : print pairs with ref month equal to [val] (e.g., 09, 10, etc)"
 echo "-p[val1/val2] : print successive pairs between val1 and val2; val1 and val2 should be in yyyymmdd format. UNTESTED. Consider combining -r, -R, -s, -S switches"
 echo "-q[val] : print pairs with sec month equal to [val] (e.g., 09, 10, etc)"
 echo "-r[val] : print pairs with ref less than or equal to [val] in yyyymmdd format"
 echo "-R[val] : print pairs with ref greater than or equal to [val] in yyyymmdd format"
 echo "-s[val] : print pairs with sec less than or equal to [val] in yyyymmdd format"
 echo "-S[val] : print pairs with sec greater than or equal to [val] in yyyymmdd format"
 echo "-w[val] : assign a filter wavelength of [val] in m"
 echo "Example - all pairs with ref beginning in 2014 or later with wavelength of 100 m with perpendicular baseline less than or equal to 60 m"
...
while getopts ":f:b:R:S:r:o:s:d:l:q:e:g:p:w:n:" opt; do
  case $opt in
    f) 
    ...
    b)
      ...
      ;;
    # 1)
    #   echo "print pairs with ref equal to $OPTARG" >&2
    #   awk -v var=$OPTARG '$1 == var {print ;}' PAIRSmake.txt  > pairs.tmp
    #   cp pairs.tmp PAIRSmake.txt
    #   ;; 
    # 2)
    #   echo "print pairs with sec equal to $OPTARG" >&2
    #   awk -v var=$OPTARG '$2 == var {print ;}' PAIRSmake.txt  > pairs.tmp
    #   cp pairs.tmp PAIRSmake.txt
    #   ;;
    o)
      echo "print pairs with ref month equal to $OPTARG" >&2
    ...
    #  echo "print pairs with ref month equal to $OPTARG" >&2
     ...
     # fi
    q)
      echo "print pairs with sec month equal to $OPTARG" >&2
      #mast_month=`echo $OPTARG | awk '{print substr($1, 5, 2)}'`
      awk -v var=$OPTARG 'substr($2, 5, 2) == var {print ;}' PAIRSmake.txt  > pairs.tmp
      cp pairs.tmp PAIRSmake.txt
      ;;
    r)
      echo "print pairs with ref less than or equal to $OPTARG" >&2
      awk -v var=$OPTARG '$1 <= var {print ;}' PAIRSmake.txt  > pairs.tmp
      cp pairs.tmp PAIRSmake.txt
      ;;
    R)
      echo "print pairs with ref greater than or equal to $OPTARG" >&2
      awk -v var=$OPTARG '$1 >= var {print ;}' PAIRSmake.txt  > pairs.tmp
      cp pairs.tmp PAIRSmake.txt
      ;;
    s)
      echo "print pairs with sec less than or equal to $OPTARG" >&2
      awk -v var=$OPTARG '$2 <= var {print ;}' PAIRSmake.txt  > pairs.tmp
      cp pairs.tmp PAIRSmake.txt
      ;;
    S)
      echo "print pairs with sec greater than or equal to $OPTARG" >&2
      awk -v var=$OPTARG '$2 >= var {print ;}' PAIRSmake.txt  > pairs.tmp
      cp pairs.tmp PAIRSmake.txt
      ;;
    d)
    ...
    p)
      ...
      echo "print successive pairs between $tstart and $tend WARNING - untested. Consider combining -r, -R, -s, -S switches"
