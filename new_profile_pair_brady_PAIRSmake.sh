...
then
  ...
  echo "args: new_profile_pair_brady.sh [phase_grd_file (with .grd)] [quantity (e.g. phase, phase-filtered)] [name of PAIRSmake file]"
  exit 1 
fi
...
tcsh -f ~ebaluyut/bin_htcondor/plot_grd_e_lesley.csh $pha1 $quantity $type $a $sat $track $site $dt $bperp
