...
then
  ...
  makecpt -T-15/30/0.1 -Cpolar -D -I > cpt.cpt # unwrapped phase plot
  if [[ $isutm == 0 ]] # if not UTM, plot with file's region, dlat, and dlon
  then
...
then
  makecpt -T-1415/1415/50 -Cpolar -D > cpt.cpt # unwrapped phase plot
  if [[ $isutm == 0 ]] # if not UTM, plot with file's region, dlat, and dlon
  then
  ...
  psscale -C./cpt.cpt -D3.5/-1/6/0.1h -Baf1+l" (m^3)" -O -K  >> ${outfile}
  rm cpt.cpt
...
# -Bx${dlon_km}+u"km" -By${dlat_km}+u"km"
  psbasemap -JX7/12 $region_km  -By${dlat_km}+u"km" -Bx${dlon_km}+u"km" -BWSen+t"$sat1 $trk1" -P -K -O >> ${outfile}
fi
