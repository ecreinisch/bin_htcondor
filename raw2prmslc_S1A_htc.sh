...
scp $maule:/s21/insar/S1A/${trk}/preproc/S1A${mast}_${satparam}.tgz ${mast}.tgz
scp $maule:/s21/insar/S1A/${trk}/preproc/S1A${slav}_${satparam}.tgz ${slav}.tgz
...
mbase_name=`find S1A${mast}*${satparam}/annotation/s1a*00${satparam}.xml | awk -F/ '{print $(NF)}' | awk -F. '{print $1}'`
...
sbase_name=`find S1A${slav}*${satparam}/annotation/s1a*00${satparam}.xml | awk -F/ '{print $(NF)}' | awk -F. '{print $1}'`
...
ln -s S1A${mast}*${satparam}/measurement/s1a*00${satparam}.tiff .
ln -s S1A${mast}*${satparam}/$mEOF_file .
ln -s S1A${slav}*${satparam}/measurement/s1a*00${satparam}.tiff .
