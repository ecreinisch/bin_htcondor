#!/bin/bash
# script for plotting 1 pair with footer information for center plots in a panel 
# Elena C Reinisch 20170520

if [[ $# -eq 0 ]]
then
echo "script for plotting 1 pair with footer information for center plots in a panel"
...
then
  ...
  makecpt -T-20/30/0.1 -Cpolar -D -I > cpt.cpt # unwrapped phase plot
  if [[ $isutm == 0 ]] # if not UTM, plot with file's region, dlat, and dlon
  then
    grdimage r2mm.grd -Y7 -C${cdir}/cpt.cpt -JX50/10 $region -P  -Bwsne -O -K > ${outfile}
  else
