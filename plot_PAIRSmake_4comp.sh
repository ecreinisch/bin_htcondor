...
while read -r line; do
   ...
   count=1
   for pfile in $pha1 $pha2 $pha3 $pha4; do
     if [[ -e $pair/${pfile}.grd ]]
     then
 	...
 	1)
        ...
        3)
          ...
          then
            ...
            ~ebaluyut/bin_htcondor/plot_pair_panel_cs.sh $sat $trk $site Residual $pair/${pfile}.grd ${pair}_${pfile}_${count}-tmp.ps $mmperfringe $bperp $user $filter_wv $dt $demf
          fi
          ;;
        4)
          if [[ ${pfile} == *"volume_change"* ]]
          then
            ~ebaluyut/bin_htcondor/plot_pair_panel_rs.sh $sat $trk $site Volume_change $pair/${pfile}.grd ${pair}_${pfile}_${count}-tmp.ps $mmperfringe $bperp $user $filter_wv $dt $demf
          else
  ...
  convert -density 300 ${pair}_${pha1}_1-tmp.pdf ${pair}_${pha2}_2-tmp.pdf ${pair}_${pha3}_3-tmp.pdf ${pair}_${pha4}_4-tmp.pdf +append tmp4.pdf 
  convert -density 300 tmp4.pdf -quality 100 ${pair}_4comp.pdf
  rm *tmp*.pdf
