 	...
 	1)
        ...
        3)
          ...
          then
            ...
            ${bin_htcondor_home}/plot_pair_panel_cs.sh $sat $trk $site Residual $pair/${pfile}.grd ${pair}_${pfile}_${count}-tmp.ps $mmperfringe $bperp $user $filter_wv $dt $demf
          fi
          ;;
        4)
          if [[ ${pfile} == *"volume_change"* ]]
          then
            ${bin_htcondor_home}/plot_pair_panel_rs.sh $sat $trk $site Volume_change $pair/${pfile}.grd ${pair}_${pfile}_${count}-tmp.ps $mmperfringe $bperp $user $filter_wv $dt $demf
          else
