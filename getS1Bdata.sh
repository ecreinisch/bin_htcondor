#!/bin/bash
# Script for querying/downloading S1B data based on Scott Baker's ssara_federated_query.py
...
if [[ $# -eq 0 ]]
then
  echo "query and download S1B data"
  ...
  echo " getS1Bdata.sh brady 2016-10-01 2016-12-01 k"
  echo "Example, with download and track specified:"
  echo " getS1Bdata.sh brady 2016-10-01 2016-12-01 144 d"
  echo "Example, with download, track, and frame specified:"
  echo "getS1Bdata.sh fuego 2018-01-01 2018-06-04 136/43 d"
  exit 1
fi
...
while [[ $lcount -lt 6 && $qcount -eq 0 ]]; do
  ssara_federated_query.py --platform=SENTINEL-1B --intersectsWith="$polygon" ${trk} ${frame} --asfResponseTimeout=25 -s $tstart -e $tend --kml
...
if [[ "$status" == "download" ]]
then
#  ssara_federated_query.py --platform=SENTINEL-1B --intersectsWith="$polygon" $trk --asfResponseTimeout=25 -s $tstart -e $tend --$status
...
#preS1B_htc.sh $trk $site newTransfer.lst
#mv tmp transfer-`date +%Y%m%d_%H%M`
#
#exit 
#
## get initial PRM files for egenerate_pairlist
#tmp_preproc_S1B.sh $trk $site newTransfer.lst
#
## update pairlist
#mkdir -p ../preproc
#cd ../preproc
